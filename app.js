...
url=initialzeQueries(url,/*put here what you wanna search about:*/"iphone 12");
async function scrape(url,output) {
  ...
  const allArticles = await page.evaluate(() => {
    ...
    return Array.from(articles).map((article) => {
      ...
      const oldPrice = article.querySelector("div.info div.s-prc-w .old")?.innerText;
      ...
      return { title, curretPrice, oldPrice, discount, rating, image, link, productHtml };
    });
  });
